...
plt.rcParams.update({
    ...
    'grid.color': 'gray',       
    'grid.alpha': 0.5
    # 'grid.linewidth': 0.9 for the second case, less clear image, legible image
})
...
def evaluate_preds_simple(y_true, y_pred):
...
def evaluate_preds(y_true, y_pred, y_pred_lower, y_pred_upper):
...
def plot_predicted_interval_dl(y_test, y_pred, y_pred_lower, y_pred_upper, dates, title):

    y_pred = y_pred.flatten() if y_pred.ndim > 1 else y_pred
    y_test = y_test.flatten() if y_test.ndim > 1 else y_test
    y_pred_lower = y_pred_lower.flatten() if y_pred_lower.ndim > 1 else y_pred_lower
    y_pred_upper = y_pred_upper.flatten() if y_pred_upper.ndim > 1 else y_pred_upper
...
def create_dl_sequences(df: pd.DataFrame, 
                        target_column: str,
                        selected_columns: list,
                        window_size: int):
    """
    Convert a multivariate time series into sequences for deep learning.

    Args:
        df (pd.DataFrame): Time-indexed multivariate time series including the target column.
        target_column (str): Name of the target variable to forecast (e.g., "SPI").
        selected_columns (list): List of columns to use as input features, including the target.
        window_size (int): Number of past time steps to include in each input sequence.

    Returns:
        X (np.ndarray): Input array of shape (num_samples, window_size, num_features).
        y (np.ndarray): Target array of shape (num_samples,).
    """
    X, y = [], []
    for i in range(len(df) - window_size):
        window = df.iloc[i:i+window_size][selected_columns].values
        target = df.iloc[i + window_size][target_column]
        X.append(window)
        y.append(target)
